...
use UnitedCMS\CoreBundle\Collection\CollectionParameterBag;
...
class CollectionParameterBagTest extends TestCase
{

    public function testGetterAndSetter() {
        $bag = new CollectionParameterBag();
    ...
    public function testSelectModeIndicator() {
        $bag = new CollectionParameterBag();
        $bag->setSelectMode(CollectionTypeInterface::SELECT_MODE_SINGLE);
        $this->assertTrue($bag->isSelectModeSingle());
        $this->assertFalse($bag->isSelectModeNone());

        $bag->setSelectMode(CollectionTypeInterface::SELECT_MODE_NONE);
    ...
    public function testCreateFromCollection() {
        ...
        $collection->setIdentifier('co1')->setContentType(new ContentType())->getContentType()
        ...
        $this->assertEquals(json_encode([
            ...
            'select' => [
                'mode' => CollectionTypeInterface::SELECT_MODE_SINGLE,
                'is_mode_none' => false,
                'is_mode_single' => true,
            ],
            'settings' => [
        ...
        ]), json_encode(CollectionParameterBag::createFromCollection(
            $collection,
            $generator,
            CollectionTypeInterface::SELECT_MODE_SINGLE,
            ['foo' => 'baa']
        )));
    }
}